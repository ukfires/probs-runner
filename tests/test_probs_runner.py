...
def test_very_simple_query(tmp_path):
    ...
    result = probs_convert_and_query_data(
        [source], ["SELECT ?p ?o WHERE { :Farming ?p ?o }"], working_dir=tmp_path
    )[0]
...
def test_query_data(tmp_path):
    ...
    output_filename = tmp_path / "output.nt.gz"
    probs_convert_data(
        [source], output_filename, working_dir=tmp_path / "working_conversion"
    )

    # Now query the converted data
    query = "SELECT ?obj ?value WHERE { ?obj :hasValue ?value } ORDER BY ?obj"
    result = probs_query_data(
        output_filename, [query], working_dir=tmp_path / "working_reasoning"
    )[0]
...
def test_probs_endpoint(tmp_path):
    ...
    with probs_endpoint(
        output_filename, working_dir=tmp_path / "working_reasoning"
    ) as rdfox:
